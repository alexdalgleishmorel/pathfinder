...
export class GridDataService {
  ...
  private gScore: Record<number, number> = {};
  private cameFrom: Record<number, number> = {};
  private fScore: Record<number, number> = {};
  ...
  aStarPathfinder (start: number[], goal: number[], rows: number, cols: number) {
      ...
      this.gScore[this.coordToNumber(start)] = 0;

      // For node n, fScore[n] := gScore[n] + h(n). fScore[n] represents our current best guess as to
      // how short a path from start to finish can be if it goes through n.
      this.fScore[this.coordToNumber(start)] = this.calcHscore(start, goal);

      while (!openQueue.isEmpty()) {
          let current = openQueue.dequeue();
          if (current.element[0] === goal[0] && current.element[1] === goal[1]) {
              return this.reconstructPath(this.cameFrom, current.element);
          }
          let neighbors = this.getNeighbors(current.element, rows, cols);
          for (var i = 0; i < neighbors.length; i++) {
              let neighbor = this.coordToNumber(neighbors[i]);
              // tentative_gScore is the distance from start to the neighbor through current
              let tentative_gScore = this.gScore[this.coordToNumber(current.element)] + 1;
              if (tentative_gScore < this.gScore[neighbor]) {
                  // This path to neighbor is better than any previous one. Record it!
                  this.cameFrom[neighbor] = this.coordToNumber(current.element);
                  this.gScore[neighbor] = tentative_gScore;
                  this.fScore[neighbor] = this.gScore[neighbor] + this.calcHscore(neighbors[i], goal);
                  if (!openQueue.contains(neighbors[i], this.fScore[neighbor])) {
                      openQueue.enqueue(neighbors[i], this.fScore[neighbor]);
  ...
  getNeighbors(node: number[], rows:number, cols:number) {
      let x = node[0];
      let y = node[1];
      ...
      for (var i = 0; i < 4; i++) {
          ...
          else if (!(this.gScore[this.coordToNumber(neighbors[i])])) {
              // assigning infinite gScore as this node is being instantiated
              this.gScore[this.coordToNumber(neighbors[i])] = Infinity;
          }
  ...
  reconstructPath(cameFrom: {}, current: number[]) {
    let total_path = [current];
    while (this.cameFrom[this.coordToNumber(current)]) {
        // Add parent of node to total path list
        current = this.numberToCoordinate(this.cameFrom[this.coordToNumber(current)]);
